...
const ListCard = ({ card, index, updateCard }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://snipscript-3.wl.r.appspot.com/api/snippet/${card.id}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        const response = await fetch(`https://snipscript-3.wl.r.appspot.com/api/snippet/tags/${card.id}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
